...
class CitrusGateway implements PaymentGatewayInterface {
    ...
    {
        $this->vanityUrl = Config::get('payment.citrus.vanityUrl');
        $this->secretKey = Config::get('payment.citrus.secretKey');
        ...
        $this->parameters['returnUrl'] = url(Config::get('payment.citrus.returnUrl'));
        $this->parameters['notifyUrl'] = url(Config::get('payment.citrus.notifyUrl'));
    }
    ...
    public function send()
    {
        ...
        return View::make('payment::citrus')->with('hash',$this->hash)
