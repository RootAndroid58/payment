...
class PayUMoneyGateway implements PaymentGatewayInterface {
    ...
    {
        $this->merchantKey = Config::get('payment.payumoney.merchantKey');
        $this->salt = Config::get('payment.payumoney.salt');
        ...
        $this->parameters['surl'] = url(Config::get('payment.payumoney.successUrl'));
        $this->parameters['furl'] = url(Config::get('payment.payumoney.failureUrl'));
    ...
    public function send()
    {
        ...
        return View::make('payment::payumoney')->with('hash',$this->hash)
